...
def load_tinyimagenet(data_dir: str) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor, List[str]]:
    ...
    """
    train_dir = os.path.join(data_dir, Path(r'C:\Users\Ivan\Desktop\Asignatures5tcarrera\TFG\codi\Fine-Tuning-MNV2_v2\tinyImgNet-train'))
    val_dir = os.path.join(data_dir, Path(r'C:\Users\Ivan\Desktop\Asignatures5tcarrera\TFG\codi\Fine-Tuning-MNV2_v2\tinyImgNet-val'))
...
def _load_train_data(train_dir: str, class_to_index: dict) -> Tuple[tf.Tensor, tf.Tensor]:
    ...
    for class_name, class_idx in class_to_index.items():
        ...
        for img_name in os.listdir(class_path):
            ...
            img = tf.image.resize(img, [DIMX, DIMY])         CHAMANURA avall pa vore si millora la cosa
